...
if(isset($_POST['signupSubmit'])){
    ...
    if(!empty($_POST['email'])){
        $respuestaMail=RecuperarPassword::verificarMail($_POST["email"]);
        if($respuestaMail=="success"){
            ...
            $resetPassLink = 'https://'.$_SERVER["SERVER_NAME"].'/SIRA/recuparar/resetpassword.php?fp_code='.$respuestaId["idpersonal"];
				...
				<br/>Para restablecer su contraseña, visite el siguiente enlace: <a href="'.$resetPassLink.'" class="btn btn-warning btn-block">Verificar</a>
				<br/><br/>Saludos,
				<br/>Sistema de renta de autos';
				//set content-type header for sending HTML email
				$headers = "MIME-Version: 1.0" . "\r\n";
				$headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
				//additional headers
				$headers .= 'From(de): Sistema de renta de autos (rentalsys)>' . "\r\n";
				//send email
